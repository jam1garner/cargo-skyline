...
fn main() {
    ...
    let result = match subcommand {
        Install { ip, title_id, debug, git, features, no_default_features } => if let Some(git) = git {
            installer::from_git(&git, ip, title_id, !debug, features, no_default_features)
        } else {
            installer::install(ip, title_id, !debug, features, no_default_features)
        },
        ...
        Check => build::check(),
        Clippy => build::clippy(),
        Run { ip, title_id, debug, restart , features, no_default_features }
            => installer::install_and_run(ip, title_id, !debug, restart, features, no_default_features),
